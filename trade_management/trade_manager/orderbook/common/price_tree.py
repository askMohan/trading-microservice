...
class PriceTree(object):
    ...
    def update_order_price_and_order_price_list(self, id, curr_price, new_price):
        if self.order_exists(id):
            curr_price_list = self.price_map[curr_price]
            order = self.order_map[id]
            curr_price_list.remove(order)
            order.price = new_price
            new_price_list = self.price_map[curr_price]
            new_price_list.add(order)
            return order
        return None
    
    def remove_order_from_price_tree_and_order_price_list(self, id, curr_price):
        if self.order_exists(id):
            curr_price_list = self.price_map[curr_price]
            order = self.order_map[id]
            curr_price_list.remove(order)
    ...
    def match_price_order(self, curr_order):
        ...
        while ((curr_order.is_bid and curr_order.price >= best_price)
                ...
                and curr_order.quantity > 0:
            # Get price OrderList
            matching_orders_list = self.get_price_list(best_price)
